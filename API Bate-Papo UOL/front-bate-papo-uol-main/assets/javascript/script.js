...
function registrarParticipante() {
  ...
  const requisicao = axios.post('http://localhost:5009/participants', dados);
  requisicao
    .then(response => {
...
function carregarMensagens() {
  ...
  const requisicao = axios.get('http://localhost:5009/messages?limit=50', {
    headers: {
      User: nome
    }
  });
...
function carregarParticipantes() {
  const requisicao = axios.get('http://localhost:5009/participants', {
    headers: {
      User: nome
    }
  });
...
function agendarAtualizacaoDeStatus() {
  setInterval(atualizarStatus, 5009);
}

function atualizarStatus() {
  axios.post(
    'http://localhost:5009/status',
    {},
    {
      headers: {
        User: nome
...
function enviarMensagem() {
  ...
  const requisicao = axios.post('http://localhost:5009/messages', dados, {
    headers: {
      User: nome
    }
  });
...
function renderizarMensagens() {
  ...
  setTimeout(
    () => document.querySelector('.mensagens-container li:last-child').scrollIntoView(),
    0
  );
}
...
function enviarMensagemEditada(event, id) {
  if (event.key === 'Escape') {
    ...
    axios
      .put(
        `http://localhost:5009/messages/${id}`,
        {
          to: mensagem.to,
          text: newMessage,
          type: mensagem.type
...
function excluirMensagem(id) {
  ...
  if (confirmacao) {
    axios
      .delete(`http://localhost:5009/messages/${id}`, {
        headers: {
          User: nome
        }
      })
